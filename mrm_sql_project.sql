...
SELECT DISTINCT name,
	   CASE WHEN Facilities.facid IN (0,1) THEN CONCAT(firstname, ' ', surname)
...
SELECT name,
	...
	AND ((Bookings.memid = 0 AND guestcost > 30.0) OR (Bookings.memid != 0 AND membercost > 30.0))
	ORDER BY cost DESC
...
SELECT name,
	...
	JOIN Facilities
	ON Facilities.facid = Bookings.facid
	WHERE starttime IN (SELECT starttime FROM Bookings WHERE starttime >= '2012-09-14' AND starttime < '2012-09-15')
	AND ((Bookings.memid = 0 AND guestcost > 30.0) OR (Bookings.memid != 0 AND membercost > 30.0))
	ORDER BY cost DESC
...
SELECT name,
       SUM(CASE WHEN memid = 0 THEN guestcost
