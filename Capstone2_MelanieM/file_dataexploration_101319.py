...
def import_file(csv_file):
	all_posts = pd.read_csv(csv_file, index_col = 0, sep='|') #reads csv file
	return all_posts
...
def train_and_fit(csv_file, alphas_list):
	vectorizer = CountVectorizer(min_df = 5, tokenizer = dummy_fun, #make CountVectorizer
		preprocessor=dummy_fun, ngram_range=(1,2))
	final_posts_df = get_all_titles_final(csv_file) #get all titles
...
clf, vectorizer = train_and_fit('all_posts_reddit_onionandnotonion_2.csv', [.1, 1, 5, 10, 50])
filename = 'pickled_model_bigrams.pkl'
outfile = open(filename, 'wb')
pickle.dump(clf, outfile)
outfile.close()
filename2 = 'pickled_model_bigrams_vectorizer.pkl'
outfile2 = open(filename2, 'wb')
pickle.dump(vectorizer, outfile2)
outfile2.close()
