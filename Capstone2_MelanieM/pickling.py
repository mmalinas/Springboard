...
class Predictor:
	...
	def preprocessing(self, title):
		...
		title_noentities = self.remove_entities(title)
	...
	def get_all_titles_final(self):
		...
		all_titles_ents = all_posts['title'].apply(lambda x: self.preprocessing_entities(x))
	...
	def get_best_alpha(self):
		...
		for alpha in self.alphas_list:
			vectorizer = CountVectorizer(min_df = 5, tokenizer = self.dummy_fun, \
				preprocessor=self.dummy_fun, ngram_range=(1,2))
			final_posts_df = self.get_all_titles_final()
			Xthis, ythis = self.make_xy_bigrams(final_posts_df, vectorizer)
			...
			score = self.cv_score(clf, X_train_this, y_train_this, f1_scorer)
			if score > maxscore:
				maxscore = score
				best_alpha = alpha
		#print(best_alpha)
	...
	def save(self, filename_clf, filename_vectorizer):
		"""Save to a file"""
		clf, vectorizer = self.train_and_fit()
		f1 = open(filename_clf, 'wb')
		pickle.dump(clf,f1)
		f1.close()
		f2 = open(filename_vectorizer, 'wb')
		pickle.dump(vectorizer,f2)
		f2.close()
	
	def load(filename_clf, filename_vectorizer):
		"""Return a thing loaded from a file"""
		f1 = open(filename_clf, 'rb')
		obj1 = pickle.load(f1)
		f1.close()
		f2 = open(filename_vectorizer, 'rb')
		obj2 = pickle.load(f2)
		f2.close()
		return obj1, obj2
...
if __name__ == "__main__":
	foo = Predictor('all_posts_reddit_onionandnotonion_2.csv', [.1, 1, 5, 10, 50])
	foo.save('clf_pickle_bigrams.pickle','vectorizer_pickle_bigrams.pickle')
	print('success!')
